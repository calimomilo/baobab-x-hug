...
<template>
    ...
    <section v-if="props.type === 'donor'" id="donor" class="relative h-[100vh] bg-brand-sage-100 flex flex-col justify-center items-center px-8 py-10 gap-x-10 gap-y-8 lg:px-40 font-medium md:flex-row">
        ...
        <div class="flex flex-col gap-8 items-center md:items-start">
            ...
            <div class="flex flex-col gap-x-8 gap-y-4 md:flex-row">
                ...
                <a :href="`/${props.displayData?.slug}/donor-share?user_uuid=${user_uuid}&collect_id=${props.collect?.id}`" download class="font-semibold bg-brand-sage-200 hover:bg-brand-sage-300 active:bg-brand-sage-400 h-11 rounded px-3 flex justify-center items-center">Partager sur les réseaux</a>
            </div>
    ...
    <section v-else id="supporter" class="relative h-[100vh] bg-brand-sage-100 flex flex-col justify-center items-center px-8 py-10 gap-x-10 gap-y-8 lg:px-40 font-medium md:flex-row">
        ...
        <div class="flex flex-col gap-8 items-center md:items-start">
            ...
            <div class="flex flex-col gap-x-8 gap-y-4 md:flex-row">
                <a :href="`/${props.displayData?.slug}/supporter-share?user_uuid=${user_uuid}&collect_id=${props.collect?.id}`" download class="text-white text-center font-semibold bg-brand-rose-400 hover:bg-brand-rose-500/100 active:bg-brand-rose-600/100 h-11 rounded px-3 flex justify-center items-center">Partager sur les réseaux</a>
            </div>
            <Link :href="`/${props.displayData?.slug}`" class="text-sm md:text-md underline text-brand-indigo-700 hover:text-brand-indigo-500">Retourner à l'accueil</Link>
        </div>
    </section>
